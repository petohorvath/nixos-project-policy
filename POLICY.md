...
Root `nix flake check` runs applicable non-VM checks for the host platform. Provide nonempty host checks under the committed lock as well as under the shared compatibility pins. Keep VM execution and its build dependencies outside default checks. Provide explicit VM commands where applicable. Each member declares at least one Nix system in its `required_architectures` policy caller input. Required policy CI covers relevant evaluation/unit/integration checks on every recorded architecture, with pinned stable and unstable compatibility where relevant. Applicable VM suites gate merges on the member's selected `vm_architecture`, independently of that selection. This optional literal Linux Nix system defaults to `x86_64-linux` for existing callers. Documentation-only PRs may use relevant documentation, formatting, and policy checks.
