...
Each root flake declares its supported `systems` and applicable outputs explicitly. The stable input is `nixpkgs`; the unstable input is `nixpkgs-unstable`. Local helpers implement tools and checks, while the root remains the visible public interface. The default shell, root `.envrc`, formatter, and ordinary checks provide consistent development commands across projects.
