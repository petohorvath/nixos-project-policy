...
Update policy links, the reusable-workflow reference, `policy_version`, and the current `policyVersion` record together. Trial both compatibility channels on x86_64 Linux and aarch64 Linux and perform an end-to-end member run before activation. Inspect actual GitHub statuses on the reviewable member PR, including `Compatibility (stable, x86_64-linux)`, `Compatibility (unstable, x86_64-linux)`, and their aarch64 counterparts. Caller job prefixes are part of the exact names. Register only the observed names in `requiredChecks`, retain the ordinary policy and applicable VM gates, and obtain separate authorization before changing GitHub merge settings. A predicted name or passing policy-repository fixture does not establish a member gate.
