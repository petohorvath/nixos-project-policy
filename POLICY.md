...
Prepare weekly pin candidates, with urgent candidates when needed. A routine central PR proposes the pair and future approval together, validates every enrolled member with trusted checking authority, and changes approval only through human-reviewed merge. Keep normal member CI on current approval while the PR is open. A root-only routine batch can become complete in that same PR without member lock, version, or caller changes. Track actual member fixes and separate merges as a batch, including temporary differences and exact tested revisions. Candidate compatibility uses the registered pair through the policy runner; the separate default check uses committed locks. See the [maintenance procedure](docs/maintenance.md).
