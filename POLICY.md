...
This document defines the shared requirements. The [design](docs/normalization-design.md) explains their architecture and rationale. Member projects link to this document at their selected policy release. The v0.4.0 contract is prepared and must be published before members select it. Members own release selection and settings in their policy caller; central records own enrollment and [pin approval](https://github.com/petohorvath/nixos-project-policy/blob/main/policy/pins.json). [Legacy project records](https://github.com/petohorvath/nixos-project-policy/blob/main/policy/projects.json) remain available to supported older checkers. Copies bundled with a release are historical snapshots.
...
Retain the complete legacy record surface while supported immutable older checkers require it, including adopted entries' full check lists and identities referenced by historical batches. A migrated member's current declaration does not rewrite those compatibility records. Publishing a newer release does not retire an older one; retirement needs an explicit decision and migration period. Remove obsolete legacy records only after affected old releases are retired and their members have migrated.
