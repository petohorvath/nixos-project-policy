...
Each member owns its flake, tools, tests, lockfiles, and releases. Member flakes do not import the policy repository. The checker accepts member checkouts as test subjects without making them policy flake dependencies.
...
| Member caller           | Policy selection, required architectures, VM targets and architecture, and additional checks |
