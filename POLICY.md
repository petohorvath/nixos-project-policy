...
Name the member caller job `Policy`. The selected release defines the mandatory status names in `policy/requirements.json`: verification of the policy version and shared pins, plus separate compliance, formatting/lint, and project-test jobs for each supported Linux architecture. Require the VM status when the member has declared VM targets. Keep these jobs independent after the shared release and record snapshot, so a lint failure does not prevent project tests from running. Members may add required checks but cannot omit or rename the policy minimum. Record the full required set during adoption and verify it against GitHub merge gates; the [checker reference](docs/checker.md#ci-integration) lists the exact names.
